...
func _on_ButtonTestVector1_pressed():
	var v1 = get_v1()
	$TestVector1/Norm.text = str(LinAlgGD.norm_v(v1))
...
func _on_ButtonTestVector2_pressed():
	...
	$TestVector2/Ewise.text = str(LinAlgGD.ewise_vv_add(v1, v2))
	$TestVector2/Dot.text = str(LinAlgGD.dot_vv(v1, v2))
	$TestVector2/Dyadic.text = str(LinAlgGD.dyadic(v2))
...
func _on_ButtonTestMatrix_pressed():
	...
	$TestMatrix/Dot.text = str(LinAlgGD.dot_mv(M, v1))
	$TestMatrix/Square.text = str(LinAlgGD.dot_mm(M, M))
	var QR = LinAlgGD.qr(M)
	$TestMatrix/Q.text = str(QR[0])
	$TestMatrix/R.text = str(QR[1])
	$TestMatrix/invQR.text = str(LinAlgGD.dot_mm(QR[0], QR[1]))

	var eig_pi = LinAlgGD.eigs_powerit(M.duplicate(true))
